...
use overload ();
use vars qw(%seen %refcnt @dump @fixup %require $CLASS_PATTERN $DEBUG $HANDLE_CIRCULAR $TRY_BASE64 @FILTERS $INDENT $SORT_KEYS $REMOVE_PRAGMAS);

our $CLASS_PATTERN;
$CLASS_PATTERN ||= $ENV{"PERL_DATA_DUMP_SKIPOBJECTS_CLASS_PATTERN"};
$CLASS_PATTERN ||= qr//;
...
sub _dump
{
    ...
    unless ($dont_remember || !$HANDLE_CIRCULAR) {
	if (my $s = $seen{$id}) {
	    my($sname, $sidx) = @$s;
	    $refcnt{$sname}++;
